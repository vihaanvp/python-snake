...
def draw_cell(surface, colour, col, row, inset=1):
    rect = pygame.Rect(col * GRID_SIZE + inset, row * GRID_SIZE + inset, GRID_SIZE - 2 * inset, GRID_SIZE - 2 * inset)
    ...
    def draw(self, surface):
        ...
        for i, (col, row) in enumerate(self.body):
            if i == 0:
                ...
                draw_cell(surface, (0, max(60, 180 - i * 3), max(0, 100 - i * 2)), col, row, inset=1)
        font = pygame.font.SysFont("monospace", 16, bold=True)
        surface.blit(font.render(f"SCORE: {self.score}", True, GREEN), (10, 10))
        ...
        if self.won:
            ...
            draw_text(surface, "YOU WIN!", 52, YELLOW, WINDOW_WIDTH // 2, WINDOW_HEIGHT // 2 - 30)
            draw_text(surface, f"Score: {self.score}  Press SPACE to continue", 20, WHITE, WINDOW_WIDTH // 2, WINDOW_HEIGHT // 2 + 30)
        if self.game_over:
            overlay = pygame.Surface((WINDOW_WIDTH, WINDOW_HEIGHT), pygame.SRCALPHA)
            overlay.fill((0, 0, 0, 180))
            surface.blit(overlay, (0, 0))
            draw_text(surface, "GAME OVER", 48, RED, WINDOW_WIDTH // 2, WINDOW_HEIGHT // 2 - 30)
            draw_text(surface, f"Score: {self.score}", 24, WHITE, WINDOW_WIDTH // 2, WINDOW_HEIGHT // 2 + 10)
            draw_text(surface, "Press SPACE to restart  |  ESC for menu", 18, WHITE, WINDOW_WIDTH // 2, WINDOW_HEIGHT // 2 + 50)
...
def high_scores_screen(surface, clock, score_keeper):
    ...
    while True:
        ...
        if not scores:
            draw_text(surface, "No scores yet. Play a game!", 20, GREY, WINDOW_WIDTH // 2, WINDOW_HEIGHT // 2)
        else:
            ...
            for i, entry in enumerate(scores):
                ...
                surface.blit(score_font.render(f"#{i + 1}", True, colour), (130, y))
